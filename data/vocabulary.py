...
class NLIVocabulary(object):
    ...
    def __init__(self, file_or_data, specials_check=False):
        ...
        if isinstance(file_or_data, str):
            logger.info("Vocab loaded from: %s" % file_or_data)

        for attr in list(self.SPECIAL_TOKEN_MAP.values()):
            logger.debug("%s %d" % (attr, getattr(self, attr)))
    ...
    def build(filename_or_words, top_k=None, sort_by='frequency'):
        ...
        return NLIVocabulary(words_and_freqs)
