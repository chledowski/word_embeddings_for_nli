...
class ExperimentRun:
    ...
    def __init__(self, config_filepath):
        ...
        distributional_vectors_filepath = os.path.join(DATA_DIR, 'embeddings', self.config.get("data", "distributional_vectors"))
        ...
        self.final_vectors_filepath = os.path.join(DATA_DIR, 'embeddings', self.config.get("output", "final_vectors"))
    ...
    def initialise_model(self, numpy_embedding):
        ...
        self.attract_cost_step_mono = attract_optimiser_mono.apply_gradients(list(zip(attract_grads_mono, tvars)))
        self.attract_cost_step_cl = attract_optimiser_cl.apply_gradients(list(zip(attract_grads_cl, tvars)))
    ...
    def attract_repel(self):
        ...
        if self.log_scores_over_time:
            ...
            fwrite_simlex = open("src/scripts/retrofitting/lear/results/simlex_scores.txt", "w")
            fwrite_wordsim = open("src/scripts/retrofitting/lear/results/wordsim_scores.txt", "w")
            fwrite_hyperlex = open("src/scripts/retrofitting/lear/results/hyperlex_scores.txt", "w")
...
def simlex_analysis(word_vectors, language="english", source="simlex", add_prefixes=True):
    ...
    pair_list = []
    if source == "simlex":
        fread_simlex=codecs.open("src/scripts/retrofitting/lear/evaluation/simlex-" + language + ".txt", 'r', 'utf-8')
    elif source == "simlex-old":
        fread_simlex=codecs.open("src/scripts/retrofitting/lear/evaluation/simlex-english-old.txt", 'r', 'utf-8')
    elif source == "simverb":
        fread_simlex=codecs.open("src/scripts/retrofitting/lear/evaluation/simverb.txt", 'r', 'utf-8')
    elif source == "wordsim":
        fread_simlex=codecs.open("src/scripts/retrofitting/lear/evaluation/ws-353/wordsim353-" + language + ".txt", 'r', 'utf-8') # specify english, english-rel, etc.
...
def hyperlex_analysis(word_vectors, language="english", source="hyperlex", distance_metric="metric-1", order=2, add_prefixes=True):
    ...
    pair_list = []
    if source == "hyperlex":
        fread_simlex=codecs.open("src/scripts/retrofitting/lear/evaluation/hyperlex.txt", 'r', 'utf-8')
    elif source == "hyperlex-nouns":
        fread_simlex=codecs.open("src/scripts/retrofitting/lear/evaluation/hyperlex-nouns.txt", 'r', 'utf-8')
    elif source == "hyperlex-test":
        fread_simlex=codecs.open("src/scripts/retrofitting/lear/evaluation/hyperlex_test.txt", 'r', 'utf-8')
    else:
        "Error with HyperLex!"
...
def main():
    ...
    try:
        ...
        print("\nUsing the default config file: src/scripts/retrofitting/lear/config/experiment_parameters.cfg\n")
        config_filepath = "src/scripts/retrofitting/lear/config/experiment_parameters.cfg"
