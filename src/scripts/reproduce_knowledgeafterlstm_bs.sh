...
declare -a NAMES=(
    "knowledge_after_lstm_${EMBEDDING}_both_al0.0_dot_frac${FRAC}_bs${BATCH_SIZE}"
    "knowledge_after_lstm_${EMBEDDING}_both_al0.1_dot_frac${FRAC}_bs${BATCH_SIZE}"
    "knowledge_after_lstm_${EMBEDDING}_both_al1.0_dot_frac${FRAC}_bs${BATCH_SIZE}"
    "knowledge_after_lstm_${EMBEDDING}_both_al5.0_dot_frac${FRAC}_bs${BATCH_SIZE}"

    "knowledge_after_lstm_${EMBEDDING}_al0.1_dot_frac${FRAC}_bs${BATCH_SIZE}"
    "knowledge_after_lstm_${EMBEDDING}_al1.0_dot_frac${FRAC}_bs${BATCH_SIZE}"
    "knowledge_after_lstm_${EMBEDDING}_al5.0_dot_frac${FRAC}_bs${BATCH_SIZE}"
)
...
do
    if [[ $(($i % $MODULO)) == $(($RUN_MODULO)) ]]; then
        ...
        python src/scripts/train_eval/train.py esim ${RESULTS_DIR} \
            ...
            --i_lambda=${LAMBDA[$i]} \
            --batch_sizes.snli.train=${BATCH_SIZE} \
            --no_save_model
